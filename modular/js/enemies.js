...
function startArtillerySkill(e) {
                const pl = state.player
                // 编辑器预览支持锁定技能（游戏内 lockSkillIndex 不存在，行为不变）
                const si = e.lockSkillIndex !== undefined ? e.lockSkillIndex : e.skillIndex
                if (si === 0) {
                    // 技能1：扇形散射 1-3 波
                    e.skillState = 'skill1'
            ...
            function finishArtillerySkill(e) {
                e.skillState = 'idle'
                // 编辑器预览锁定技能时保持；游戏内 lockSkillIndex 不存在 → 正常轮询
                e.skillIndex = e.lockSkillIndex !== undefined ? e.lockSkillIndex : (e.skillIndex + 1) % 3
