        ...
        (function() {
          ...
          `
            ...
            /* ─── 发射子弹 ────────────────────────── */
            // 加特林启动预热：转速低时射速慢，逐渐加速到峰值（最低 35% 射速）
            function gunAttackInterval(p) {
                if (p.gunType === 'gatling') {
                    return 1 / (p.attackSpeed * Math.max(0.35, p.gatlingSpin || 0))
                }
                return 1 / p.attackSpeed
            }
            ...
            function fireAttackBatch(target, angle, homing, consumeAmmo) {
                ...
                // 冲锋枪：每发射 4 颗才消耗 1 发弹药（ammoCounter 累计）
                if (p.magSize > 0 && consumeAmmo !== false) {
                    if (p.gunType === 'smg') {
                        p.ammoCounter = (p.ammoCounter || 0) + 1
                        if (p.ammoCounter % 4 === 0) {
                ...
                // ---- 冲锋枪：连续单发（1 攻速 tick = 1 颗），支持平行弹道；弹药每 4 颗扣 1 发 ----
                if (p.gunType === 'smg') {
                    const tmpAngles = []
                ...
                if (p.gunType === 'ultraman') {
                    ...
                    // 固定横排中部 8 个枪口，每 tick 都发射（不轮转）：
                    // 同一枪口每 tick 都有前后粒子 → 速度随机差异 → 后面的快粒子追上前面（追进效果）
                    const startBi = Math.floor((barrels - activeBarrels) / 2)
                    for (let b = 0; b < beams; b++) {
                        ...
                        for (let s = 0; s < activeBarrels; s++) {
                            ...
                            const moy = Math.sin(baseAngle) * (p.radius + 8) + Math.cos(baseAngle) * (bx + off)
                            const a = baseAngle + rand(-0.012, 0.012)
                            const spd = currentSpeed * rand(0.5, 1.5)
                            state.projectiles.push({
                                x: p.x + mox,
                                y: p.y + moy,
                                vx: Math.cos(a) * spd,
                ...
                if (p.attackTimer <= 0) {
                    if (state.controlMode === 'keyboard') {
                        ...
                        if (wantFire && !p.reloading) {
                            ...
                            p.attackTimer = gunAttackInterval(p)
                            // 加特林预热：持续射击转速提升
                            if (p.gunType === 'gatling') p.gatlingSpin = Math.min(1, (p.gatlingSpin || 0) + 0.045)
                        } else {
                            p.attackTimer = 0.05
                            // 停止射击：转速衰减
                            if (p.gunType === 'gatling') p.gatlingSpin = Math.max(0, (p.gatlingSpin || 0) - 0.03)
                        }
            ...
            function render() {
                ...
                for (const proj of state.projectiles) {
                    ...
                    if (proj.sparkDot) {
                        ...
                        ctx.beginPath()
                        ctx.moveTo(proj.x, proj.y)
                        ctx.lineTo(proj.x - Math.cos(sa) * 8, proj.y - Math.sin(sa) * 8)
                        ctx.stroke()
                        continue
                    }
            ...
            function renderHomeList() {
              ...
              `
                ...
                // 默认选中第一把已解锁的枪（防止选中未解锁武器）
                if (!state.homeSelectedGun || !isGunUnlocked(state.homeSelectedGun)) {
                    if (unlockedGuns.length > 0) {
                        state.homeSelectedGun = unlockedGuns[0]
            ...
            function selectGun(gun) {
                ...
                p.reloadTimeMult = 1
                p.ammoCounter = 0
                // 加特林移速 -30%（弹容增大后持续火力更久，移速进一步降低平衡）
                p.speedMul = p.gunType === 'gatling' ? 0.7 : 1
